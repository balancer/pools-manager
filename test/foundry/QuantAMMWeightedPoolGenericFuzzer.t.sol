...
import "../../contracts/interfaces/IQuantAMMWeightedPool.sol";
            ...
            for (uint j = 0; j < params.numTokens; j++) {
                if (i != j) {
                    ...
                    for (uint k = 0; k < params.numTokens; k++) {
                        ...
                        int256[] memory targetWeightAndMultiplier = variables
                            .dynamicData
                        ...
                        for (uint l = 0; l < targetWeightAndMultiplier.length; l++) {
                            ...
                            console.logString(
                                string.concat(
                                    "target weight and multiplier: ",
                                    vm.toString(targetWeightAndMultiplier[l])
                                )
                            );
                        }
                        ...
                        assertEq(targetWeightAndMultiplier[weightOffset], expectedTestParam.weight);
                        assertEq(
                    ...
                    assertEq(variables.dynamicData.lastInteropTime, uint40(timestamp + params.interpolationTime));
                }
            }
        }
    }
        ...
        for (uint i = 0; i < params.numTokens; i++) {
            for (uint j = 0; j < params.numTokens; j++) {
                if (i != j) {
                    ...
                    vm.assume(
                        expectedAmountOut <
                            variables.balances[variables.secondWeight.index].mulDown(
                                _min(_MAX_IN_RATIO, uint256(params.poolParams.maxTradeSizeRatio))
                            )
                    );
        ...
        for (uint i = 0; i < params.numTokens; i++) {
            for (uint j = 0; j < params.numTokens; j++) {
                if (i != j) {
                    ...
                    vm.assume(
                        expectedAmountIn <
                            variables.balances[variables.firstWeight.index].mulDown(
                                _min(_MAX_IN_RATIO, uint256(params.poolParams.maxTradeSizeRatio))
                            )
                    );
